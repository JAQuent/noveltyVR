...
The code below creates a GLM where every $\beta$-value is set to 0.884 for 36 subjects per group. The SD for the error terms are set to 1 so that the $\beta$-value is equivalent to Cohens's D of d = 0.884. 
...
```{r echo = FALSE}
BF10_tTests <- data.frame(Effect = c('N', 'N x E', 'N x M', 'N x E x M'),
                          `BF10 point` = round(tResults[, 1]),
                          `BF10 complement` = round(tResults[, 2]),
                          `cohen's D` = round(effectSizes, 2))
names(BF10_tTests) <- c('Effect', 'BF10 point', 'BF10 complement', "cohen's D")
kable(BF10_tTests)
```

This approach also allowed us to calculate the effect size Cohen's D for each comparison.

As we can see, the choice of the null hypothesis is very important. The BF for the main effect of N comparing to the point null is `r round(round(as.numeric(as.vector(test1[2]/test1[1])))/round(as.numeric(as.vector(test1[2]))))` times higher than comparing to its complement. We don't anticipate negative effects here and values close to zero would result in inconclusive evidence (BF close to 1), we therefore decided to use the point null hypothesis. 
